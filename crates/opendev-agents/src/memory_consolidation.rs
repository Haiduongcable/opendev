...
pub async fn consolidate(working_dir: &Path) -> Option<ConsolidationReport> {
    ...
    // Acquire lock
    // Optimization: Replaced synchronous std::fs operations with tokio::fs inside this async function
    // to prevent blocking the async executor thread, improving concurrent performance.
    ...
    // Release lock
    // Optimization: Used tokio::fs::remove_file to prevent blocking the async thread.
...
async fn run_consolidation(memory_dir: &Path, backup_dir: &Path) -> Option<ConsolidationReport> {
    ...
    let write_result = {
        #[cfg(unix)]
        {
            // Optimization: Replaced synchronous std::fs writes with tokio::io::AsyncWriteExt
            // to avoid blocking the async executor during large memory file consolidations.
            // Using std_opts to safely set Unix mode before converting to tokio opts.
...
fn scan_all_memory_files(dir: &Path) -> Vec<MemoryFile> {
    ...
    files.sort_by_key(|b| b.modified);
    files
}
