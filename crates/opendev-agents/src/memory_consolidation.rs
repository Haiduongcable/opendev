...
pub async fn consolidate(working_dir: &Path) -> Option<ConsolidationReport> {
    ...
    // ⚡ Bolt Optimization: Use tokio::fs for lock acquisition instead of blocking std::fs.
    // This prevents blocking the async executor thread during file I/O operations, improving concurrency.
    let lock_result = async {
        let mut f = tokio::fs::OpenOptions::from(open_opts)
            .open(&lock_path)
            .await?;
...
async fn run_consolidation(memory_dir: &Path, backup_dir: &Path) -> Option<ConsolidationReport> {
    ...
    // ⚡ Bolt Optimization: Use tokio::fs for file writing instead of blocking std::fs.
    // Preserves the 0o600 mode on unix systems using OpenOptionsExt.
    let write_result = async {
        ...
        let mut f = tokio::fs::OpenOptions::from(opts).open(&tmp_path).await?;
        use tokio::io::AsyncWriteExt;
