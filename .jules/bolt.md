...
## 2024-05-24 - Zustand Strict Equality
**Learning:** Returning inline arrays (e.g. `[]`) as fallbacks in Zustand selectors circumvents strict equality checks, triggering re-renders even when the underlying empty state hasn't conceptually changed.
**Action:** Use a stable reference like `export const EMPTY_ARRAY = Object.freeze([]) as never[];` across the codebase for defaults.
