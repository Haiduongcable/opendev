...
## 2026-09-22 - Non-blocking startup memory consolidation checks
**Learning:** Calling `should_consolidate` (which checks file paths and reads JSON metadata from disk) on the main thread during `AgentRuntime::new` blocks the startup sequence and causes `AgentRuntime` creation to be slow, especially when disk I/O is involved.
**Action:** Move I/O heavy checks like `should_consolidate` into the `tokio::spawn` task so they execute in the background and do not delay application startup.
