...
## 2025-02-23 - Prevent O(n) chat history re-renders on new messages
**Learning:** Returning inline allocated arrays (like `[]`) from Zustand selectors (like `useChatStore`) causes the consumer component (e.g. `MessageList`, `QueueBar`) to re-render on *every* store update, because `[] !== []`.
Furthermore, `MessageItem`'s `React.memo` was breaking because `prevMessageCount` was passed as a prop from `MessageList` and updated when a new message streamed in, causing the prop to change for *all* previous `MessageItem`s in the history and triggering full list re-renders.
**Action:** Extract inline arrays into static constants outside the component so selectors return stable references (e.g., `EMPTY_MESSAGES`). When rendering lists where items need context based on the list length or current index, calculate primitive props (like `staggerDelay`) in the parent so that historical items receive unchanged props and maintain their `React.memo` optimization.
