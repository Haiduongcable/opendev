...
## 2024-05-24 - [Avoid Empty Array Instantiations in Zustand Selectors]
**Learning:** Using `?? []` as a fallback inside Zustand selectors causes new array object references to be returned every time the store updates, defeating strict equality checks and causing widespread unnecessary re-renders in components using those selectors.
**Action:** Use a stable reference like `export const EMPTY_ARRAY: never[] = Object.freeze([]) as never[];` when a fallback empty array is needed in Zustand selectors.
