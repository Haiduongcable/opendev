...
## 2024-08-04 - Unnecessary Re-renders with Zustand Store Subscriptions
**Learning:** Using `useStore()` without a selector or `useStore(state => state)` subscribes a component to every update in the entire store, causing unnecessary re-renders when unrelated state changes.
**Action:** Always use atomic selectors when accessing state from Zustand stores (e.g., `const value = useStore(state => state.value)`) to ensure components only re-render when the specific properties they depend on are updated.
