...
## 2024-08-15 - React Zustand Subscriptions
**Learning:** Full store subscriptions (e.g. `useChatStore(state => state)`) or destructuring from the store cause React components to re-render whenever ANY state in the store changes, resulting in unnecessary UI updates.
**Action:** Always use atomic selectors (e.g. `useChatStore(state => state.property)`) to restrict component re-renders to only occur when the specific mapped state changes.
