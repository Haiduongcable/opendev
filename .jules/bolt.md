...
## 2024-03-24 - Zustand Strict Equality
**Learning:** Using `?? []` as a fallback in Zustand selectors breaks strict equality checks and causes unnecessary re-renders in components because `[] !== []`.
**Action:** Use a stable empty array reference (like `EMPTY_ARRAY` from `constants/common.ts`) for fallback values in Zustand selectors.
